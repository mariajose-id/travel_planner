...
class AppLogger {
  ...
  static void initialize({
    ...
    _instance._initialize(level: level, enableFileLogging: enableFileLogging, logDirectory: logDirectory);
  }
  void _initialize({
    LogLevel? level,
    bool enableFileLogging = false,
    ...
    _initialized = true;
    getLogger('AppLogger').info('Logger initialized with level: $_currentLevel');
  }
  void _processLogEntry(LogEntry entry, bool enableFileLogging, String? logDirectory) {
  ...
  Future<void> _writeToFile(LogEntry entry, String logDirectory) async {
    try {
      ...
      final fileName = 'app_${today.year}_${today.month.toString().padLeft(2, '0')}_${today.day.toString().padLeft(2, '0')}.log';
  ...
  static void debug(String message, {String? loggerName, Map<String, dynamic>? metadata}) {
    _instance.logStructured(LogLevel.debug, loggerName ?? 'App', message, metadata: metadata);
  }
  static void info(String message, {String? loggerName, Map<String, dynamic>? metadata}) {
    _instance.logStructured(LogLevel.info, loggerName ?? 'App', message, metadata: metadata);
  }
  static void warning(String message, {String? loggerName, Map<String, dynamic>? metadata}) {
    _instance.logStructured(LogLevel.warning, loggerName ?? 'App', message, metadata: metadata);
  }
  static void error(String message, {String? loggerName, dynamic error, StackTrace? stackTrace, Map<String, dynamic>? metadata}) {
    _instance.logStructured(LogLevel.error, loggerName ?? 'App', message, error: error, stackTrace: stackTrace, metadata: metadata);
  }
  static void severe(String message, {String? loggerName, dynamic error, StackTrace? stackTrace, Map<String, dynamic>? metadata}) {
    _instance.logStructured(LogLevel.severe, loggerName ?? 'App', message, error: error, stackTrace: stackTrace, metadata: metadata);
  }
}
